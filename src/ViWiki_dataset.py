...
def get_person_ids_position(article_ids_replaced, person_token_id=40032, article_max_length=512, is_tgt_input=False):
    position_list = []
...
class ViWikiDictDatasetEntityTypeFixLenEntPos(Dataset):
    def __init__(self, data_dict, emb_dir, image_dir, art_dir , tokenizer, use_clip_tokenizer=False, entity_token_start="no", entity_token_end="no", transform=None, max_article_len=512, max_ner_type_len=80, max_ner_type_len_gt=20, retrieved_sent=False, person_token_id=40032, split = None, clip_processor=None, ):
        super().__init__()
        self.data_dict = copy.deepcopy(data_dict)
    ...
    def __getitem__(self, index):
        ...
        if self.use_clip_tokenizer:
            from transformers import AutoTokenizer
            tokenizer = AutoTokenizer.from_pretrained(r"/datastore/npl/ICEK/vacnic/vacnic_pretrained_model/clip-ViT-B-32-multilingual-v1")
            caption_ids_clip = tokenizer(
                caption,
                return_tensors="pt",
                padding=True,
...
def add_name_pos_list_to_dict(data_dict, nlp, tokenizer):
    ...
    for key, value in tqdm(data_dict.items()):
        ...
        position_list = get_person_ids_position(caption_ids_ner, person_token_id=40032, article_max_length=20, is_tgt_input=True)
