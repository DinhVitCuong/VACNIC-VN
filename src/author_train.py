...
def get_hidden_states_ner(model, src_ids, src_mask, tgt_input, img_feat, name_ids, name_mask, org_ids, org_mask, gpe_ids, gpe_mask, ner_mask):
    # stop gradient to get representation of gt ners
    # and we do not cross-attn ner to img here by setting add_img_ner_attn=False
    with torch.no_grad():
        if "," in args.gpu_ids:
        ...
        output = model(input_ids=src_ids, attention_mask=src_mask, decoder_input_ids=tgt_input, image_features=img_feat, name_ids=name_ids, org_ids=org_ids, gpe_ids=gpe_ids, name_mask=name_mask, org_mask=org_mask, gpe_mask=gpe_mask, ner_mask=ner_mask, add_img_ner_attn=False)["hidden_states_ner"]
    return output
...
if __name__ == "__main__":
    ...
    if args.only_image:
        ...
        if args.trained_clip == "no":
            model_name = f"m{args.margin}Match_{args.alpha}" + args.data_type + f"cross_newsmep_enc-self_init{args.init_attn_weight}retri{args.num_sentences}_FNID_secla{args.use_secla}_CLIPfreeze{clip_type_text}_{args.prompt_mlp_type}{args.prompt_size}{args.map_size}_{plm_type_text}_map{args.mapping_loss_weight}-{args.mapping_loss_type}_fuse{args.enc_fusion_layer}_dim{args.dim_common}_seed{args.seed}_bsz{args.train_batch_size}_lr{args.lr_bart}-{args.clip_norm}_{args.num_epoch}epoch_warm{args.warmup_rate}{args.weight_decay}_len{args.article_max_length}_bos{args.perturb}"
        else:
            model_name = f"m{args.margin}Match_{args.alpha}" + args.data_type + f"cross_newsmep_enc-self_init{args.init_attn_weight}_retri{args.num_sentences}_FNID_secla{args.use_secla}_trained_CLIPfreeze{clip_type_text}_{args.prompt_mlp_type}{args.prompt_size}{args.map_size}_{plm_type_text}_map{args.mapping_loss_weight}-{args.mapping_loss_type}_fuse{args.enc_fusion_layer}_dim{args.dim_common}_seed{args.seed}_bsz{args.train_batch_size}_lr{args.lr_bart}-{args.clip_norm}_{args.num_epoch}epoch_warm{args.warmup_rate}{args.weight_decay}_len{args.article_max_length}_bos{args.perturb}"
            if "_by" in args.trained_clip and "MENM" in args.trained_clip:
                model_name = model_name.replace("trained_CLIP", "trained_CLIP_MENM")
            elif "_by" in args.trained_clip:
                model_name = model_name.replace("trained_CLIP", "trained_CLIP_noMENM")
